...
devconsole:
	docker run -it --rm -v ${PWD}/dyndns:/usr/src/app -w /usr/src/app golang:1.8.5 bash

server_test:
	docker run -it -p 8080:8080 -p 53:53 -p 53:53/udp --env-file envfile --rm davd/docker-ddns:latest

unit_tests:
	docker run -it --rm -v ${PWD}/dyndns:/go/src/dyndns -w /go/src/dyndns golang:1.8.5 /bin/bash -c "go get && go test -v"
